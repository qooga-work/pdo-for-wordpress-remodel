	...
	*/
	...
	function query($query) {
		...
		if( strtolower(mb_substr(str_replace(" ","",$query),0,6)) == "select" ){
			$query	= preg_replace("/YEAR\(wp_posts\.post_date\)=([0-9]*[0-9])/i", "strftime('%Y',wp_posts.post_date)='$1'" , $query);
			$query	= preg_replace("/DAYOFMONTH\(wp_posts\.post_date\)=([0-9]*[0-9])/i", "strftime('%d',wp_posts.post_date)='$1'" , $query);
			$query	= preg_replace("/MONTH\(wp_posts\.post_date\)=([0-9]*[0-9])/i", "strftime('%m',wp_posts.post_date)='$1'" , $query);
		}
		// @to
		...
		if($query == "SELECT FOUND_ROWS()"){
			...
			// count(*)‚É•ÏX‚·‚é(SQL_CALC_FOUND_ROWSÁ‚·H) FOUND_ROWS()
			$query	= preg_replace("/(SELECT .* FROM)/i", "SELECT COUNT(*)  FROM" , $query);
			$query	= preg_replace("/(GROUP BY.*)/i", "" , $query);	// GROUP BY
			$query	= preg_replace("/(ORDER BY.*)/i", "" , $query);	// ORDER BY
			$query	= preg_replace("/(LIMIT [0-9]*,.*[0-9])/i", "" , $query);	// limitÁ‚·
		}else{
